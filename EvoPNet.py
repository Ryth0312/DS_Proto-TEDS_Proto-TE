...
from Encoders import EHREncoder2  # 假设你已经实现了EHREncoder2
...
class EvoPModel(nn.Module):
    def __init__(self,
        ...
        # 修改后的编码器部分
        self.encoder = EHREncoder2(
            input_size=76,  # 输入特征维度
            hidden_size=nhid,  # LSTM hidden size 对应于nhid
            attention_input_dim=2*nhid,  # 用于attention的输入维度
            attention_hidden_dim=2*nhid,  # 用于attention的hidden size
            nhead=nhead,  # 多头注意力头数
            nlayers=num_layers,  # LSTM层数
            bidirectional=bidirectional,  # 是否使用双向LSTM
            p=p  # Dropout比例
        )

        # 分类器头部
        self.classifier = nn.Linear(self.encoder.attention_hidden_dim, n_cls)  # 输出为类别数的分类器
    ...
    def forward(self, model_type, demo_data, seq_data, seq_lens, src_mask=None):
        ...
        :param seq_data: EHR数据，维度 (N, T, D_seq)
        ...
        #print(f"Input shape before LSTM: {seq_data.shape}")
        embeddings = self.encoder(seq_data)  # (N, T, hidden_dim) 由 EHREncoder2 输出
